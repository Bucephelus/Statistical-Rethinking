...
# ╔═╡ e33776a9-f39b-4089-bccf-10d1f6711dda
using Distributions, Random, Plots, DataFrames, CSV, Turing, StatsPlots, Statistics, StatisticalRethinking, Logging, BSplines

# ╔═╡ a3b2fe2f-0aa3-41ed-8b5a-817510d665c6
using OrdinaryDiffEq

# ╔═╡ 76c07912-a956-11f0-16ed-efbd1dd41ccc
md"""
#### Solutions for 4M4, 4M5, 4M6, and 4H3 (a), (b)
"""

# ╔═╡ 8e01e935-019b-4bff-b8c8-b649219fbfea
md"""
##### 4M4. 
...
# ╔═╡ 5d90df24-4bd4-43ce-8ac4-7800759fb53f
md"""
##### Solution: 


I want to predict *height* using *year* as a predictor  (given these are school students, their height should increase as they age). Thus a standard Bayesian linear regression model would be: 

$$\text{height}_{i} \sim \text{Normal}(\mu_{i}, \sigma)$$

$$\mu_{i} = \alpha + \beta \times \text{year}_{i}$$
"""

# ╔═╡ 042d6138-c7a1-4378-9ef2-d1e2feb759f5
md"""
...
- For $\alpha$, we assume the average height is around 160cm, with 15cm uncertainty. 
- For $\beta$, we are unsure of the yearly growth trend so we say around ?10cm? growth per year. We should use a positive-centred prior like Normal(10, 5) since we *know* that the students will grow taller, however we will just go ahead with (-10, 10).
- For $\sigma$, I assume the height data is standardised, and so outcomes have small resudual noise.
"""

# ╔═╡ 6021f32a-5e83-4083-8ea9-d20966d843b8
begin
	# define priors 
	α = Distributions.Normal(150, 20)
	β = Distributions.Uniform(-10, 10)
	σ = Distributions.Exponential(1)

	# simulating prior predicitive distribution
	Random.seed!(42)
	n = 100
	year = 1:3
	...
	μ = [α_samp[i] .+ β_samp[i] .* year for i in 1:n]
	height_sim = [rand.(Normal.(μ[i], σ_samp[i])) for i in 1:n]

	# visualize prior predictions
	s = plot(year, height_sim[1], label=false, xlabel="Year", ylabel="Height (cm)", title="Prior predictive lines")
	for i in 2:n
    plot!(year, height_sim[i], label=false, alpha=0.3, color=:gray)
	end
	s
end

# ╔═╡ 19e58077-af87-4bc1-bce2-6e3d4c999f98
md"""
##### 4M5. 

Now suppose I remind you that every student got taller each year. Does this information lead you to change your priors? How? 
"""

# ╔═╡ 7b2addb4-0da2-4f9c-add7-b03cac129617
md"""
We will just replace the prior for $\beta$ with Uniform(0,10). 
"""

# ╔═╡ b47e4f44-116d-4608-a039-24e881491050
md"""
##### 4M6. 

Now suppose I tell you that the variance among heights for students of the same age is never more than 64cm. How does this lead you to revise your priors? 

This implies that $\sigma < 64$. Thus we can use a prior that heavily weights smaller values. We can decrease $\sigma$ in the prior for $\alpha$ with Normal($150$, $\sqrt 64$). 

"""

# ╔═╡ 4bcaa09d-1b9c-43da-bbc0-ed5d0c8fb109
md"""
##### 4H3. 
Suppose a colleague of yours, who works on allometry, glances at the pracice problems just above. Your colleague exclaims, "That's silly. Everyone knows that it's only the logarithm of body weight that scales with height!". Let's take your colleague's advice and see what happens. 

- (a) Model the relationship between height (cm) and the natural logarithm of weight (log-kg). Use the entire Howell 1 data frame, all 544 rows, adults and non-adults. Can you interpret the resulting estimates. 
...
# ╔═╡ 3f32e02f-1719-4395-baf5-6887d0cb7c1d
begin
	md"""
	(a) 
	"""
	
	df = CSV.read("Howell1.csv", DataFrame)
...
# ╔═╡ 2486cad0-8024-4cb0-ac85-18a51adf2c61
describe(d)

# ╔═╡ 45b6030d-e052-4c3c-bb21-24a90c4e03ea
begin
	@model function model_4h3(log_w, height)
    a ~ Normal(178, 20)
    b ~ LogNormal(0, 1)
    μ = @. a + b * log_w
    σ ~ Uniform(0, 50)
	...
	m4h3 = sample(model_4h3(d.log_w, d.height), NUTS(), 1000)
	m4h3_df = DataFrame(m4h3)
	precis(m4h3_df)
end

# ╔═╡ fea9bd09-4264-419b-83ec-e3ab44912df2
md"""
Parameter interpretation: 

-$\alpha$ (intercept) - this is the expected height when log_w = 0, which in turn corresponds to a weight of $e^0 = 1\text{kg}$. Since no one actually weighs 1kg in the dataset, the intercept is out of the range of the dataset. It's also negative since it's extrapolating far below our actual data range.

-$\beta$ (gradient) - when weight increases by a factor of $e$ (~2.718), height increases by 46.8cm. This is the power-law relationship mentioned.

-$\sigma$ (standard deviation of the residuals) - after accounting for log(weight), there's still about 5cm of unexplained variation in height.
"""
...
manifest_format = "2.0"
project_hash = "2a85b0e2732fa76ebfe9e472e0617642be0c84fa"
...
git-tree-sha1 = "091c79dc9387f558ab8785a2467dbad02d30ef3b"
...
git-tree-sha1 = "07def45ab53ec4f4b2897f4162543d1085d0b38a"
